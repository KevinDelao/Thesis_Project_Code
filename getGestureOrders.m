...
% Returns a subjects*gestures array indicating the optimal order to
...
savePath = 'gestureOrders_RMS'; % Save path: ***EDIT ACCORDIGNLY***

feature = {'RMS' 'emgRMS'}; % Feature name-array_name pair - could be altered for other features

%% Variables
%change depending which DB being used
numGestures = 53;
numSubjects = 27;
...
%% Arbitary order
%create an array starting with rest then movements in order of 1 to 52 for
%each patient, this will be the default ordering from db1, the one means
%repeat only once the columns
...
%     gestureOrderMean(subject,:) = gestureSetMean;
    ...
    %rest will be the first movement in the set, so add rest first
